...
function pcaiUpdateExecutiveCommandDashboard() {
  const dash = pcaiGetOrCreateSheet_(PCAI_SHEETS.EXEC_DASHBOARD);
  ...
  const collectionsRisk = pcaiSheetExists_(PCAI_SHEETS.COLLECTIONS_RISK)
    ? pcaiGetSheetRequired_(PCAI_SHEETS.COLLECTIONS_RISK)
    : null;
  ...
  pcaiWriteTopRowsToDashboard_(collectionsRisk, dash, 23, 1, 5, 6);
...
function pcaiExportDailySummary() {
  const sh = pcaiGetOrCreateSheet_(PCAI_SHEETS.DAILY_SUMMARY);
...
function pcaiGenerateWeeklyBusinessReview() {
  const sh = pcaiGetOrCreateSheet_(PCAI_SHEETS.WEEKLY_BUSINESS_REVIEW);
  pcaiResetSheetContents_(sh);
  ...
  sh.getRange("D5:E10").setValues([
    ...
    ["Items On Reorder", ops ? ops.getRange("B7").getValue() : ""],
    ["Top Revenue Lab", ops ? ops.getRange("B8").getValue() : ""],
    ["Top Credit Risk Lab", collectionsRisk ? collectionsRisk.getRange("A4").getValue() : ""],
...
function pcaiGetWhatIsBlockingScaleToday() {
  ...
  if (pcaiSheetExists_(PCAI_SHEETS.COLLECTIONS_RISK)) {
    const sh = pcaiGetSheetRequired_(PCAI_SHEETS.COLLECTIONS_RISK);
    const creditLab = sh.getRange("A4").getValue();
...
function pcaiRunDailyMonitoringLoop() {
  ...
  pcaiBuildCollectionsRiskSummary();
