...
function pcaiOpenSidebar() {
  const html = HtmlService.createHtmlOutput(pcaiGetSidebarHtml_()).setTitle("PrimeCare AI");
...
<head>
  ...
  <style>
    ...
    *{ box-sizing:border-box; }
    ...
    .app{
      height:100vh; display:flex; flex-direction:column; gap:10px; padding:10px;
      background:
    ...
    .logo{
      ...
      color:#08101e; font-weight:900; font-size:14px;
    }
    ...
    .tbtn,.miniChip,.copy{
      border-radius:10px; padding:8px 9px; font-size:11px; cursor:pointer;
    }
    .quickGrid{ margin-top:10px; display:grid; grid-template-columns:1fr 1fr; gap:8px; }
    .chip{
      border:1px solid var(--border); background:rgba(255,255,255,.06);
      color:var(--text); border-radius:12px; padding:10px 11px; font-size:12px;
      cursor:pointer; min-height:54px; display:flex; flex-direction:column; justify-content:center;
    }
    .chip b{ color:var(--accent); }
    .statusbar{ margin-top:10px; display:flex; gap:8px; flex-wrap:wrap; }
    .pill{
      border:1px solid var(--border); background:rgba(255,255,255,.05);
      border-radius:999px; padding:6px 10px; font-size:10px; color:var(--muted);
    }
    .chatwrap{
    ...
    .msg{
      max-width:92%; border:1px solid var(--border); border-radius:14px;
      padding:10px 11px; font-size:12px; line-height:1.45; white-space:pre-wrap;
      word-break:break-word;
    }
    .user{ align-self:flex-end; background:rgba(102,227,255,.10); }
    .ai{ align-self:flex-start; background:rgba(15,26,46,.78); }
    .system{ align-self:center; background:rgba(255,255,255,.05); color:var(--muted); font-size:11px; max-width:100%; }
    .meta{
      margin-top:7px; display:flex; align-items:center; justify-content:space-between;
      gap:8px; color:var(--muted); font-size:10px;
    }
    .composer{
      border-top:1px solid var(--border); background:rgba(11,18,32,.94);
      padding:10px; display:flex; flex-direction:column; gap:8px;
    }
    .textareaWrap{ position:relative; }
    textarea{
      width:100%; min-height:76px; max-height:180px; resize:none; border-radius:14px;
      border:1px solid var(--border); background:rgba(255,255,255,.04); color:var(--text);
      padding:12px 12px 32px 12px; font-size:12px; outline:none;
    }
    .kbd{
      position:absolute; right:10px; bottom:8px; font-size:10px; color:var(--muted);
      border:1px solid var(--border); border-radius:999px; padding:2px 6px;
      background:rgba(255,255,255,.04);
    }
    .row{ display:flex; align-items:center; justify-content:space-between; gap:8px; }
    .hint{ font-size:10px; color:var(--muted); line-height:1.3; max-width:60%; }
    .btns{ display:flex; gap:8px; }
    .btn{
      border:none; border-radius:12px; padding:9px 12px; font-weight:900;
    ...
    .footerMini{ display:flex; gap:6px; flex-wrap:wrap; }
    .miniChip{ border-radius:999px; padding:5px 8px; font-size:10px; }
    .toast{
      position:fixed; left:50%; transform:translateX(-50%); bottom:14px;
      background:rgba(15,26,46,.96); border:1px solid var(--border); color:var(--text);
      border-radius:12px; padding:8px 10px; font-size:11px; display:none; max-width:92%;
    }
  </style>
</head>
<body>
  ...
  function addMsg(role, text){
    ...
    meta.innerHTML =
      role === 'user'
        ...
        : '<span>PrimeCare AI</span><span>now</span>';
    ...
    if(role !== 'system') el.appendChild(meta);
    chat.appendChild(el);
    scrollChat();
    return el;
  }
