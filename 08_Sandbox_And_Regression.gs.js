...
function pcaiSetupSandboxSimulationEngine() {
  ...
  pcaiLogActionSafe_("SETUP_SANDBOX", "Created sandbox simulation sheets");
...
function pcaiCreateOrResetTestDashboardChecks_() {
  const sh = pcaiResetSheetContents_(pcaiGetOrCreateSheetSafe_(PCAI_SHEETS.TEST_DASHBOARD));
  sh.getRange("A1")
    .setValue("Sandbox Dashboard Verification")
    .setFontWeight("bold")
    .setFontSize(16);
...
function pcaiRunSandboxScenarioByName(scenarioName) {
  ...
  if (name === "high overdue labs") {
    pcaiSimulateHighOverdueLabs_();
  } else if (name === "stock out stress") {
    pcaiSimulateStockOutStress_();
  } else if (name === "month end load") {
    pcaiSimulateMonthEndLoad_();
  } else if (name === "combined stress scenario") {
    pcaiSimulateCombinedStressScenario_();
  } else {
  ...
  pcaiLogActionSafe_("RUN_SANDBOX_SCENARIO", name);
  return "Sandbox scenario completed: " + scenarioName;
}

function pcaiSimulateHighOverdueLabs_() {
  const ar = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_AR);
  const orders = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_ORDERS);

  [
    {
      Lab_ID: "LAB_RISK_001",
      Total_Delivered: 120000,
      Total_Paid: 10000,
      Outstanding: 110000,
      Credit_Limit: 50000,
      Days_Overdue: 45,
  ...
  [
    {
      ...
      Invoice_ID: "SIM_INV_1001",
      Invoice_Status: "Sent",
      Payment_Status: "Overdue"
    },
    {
      Order_ID: "SIM_OD_1002",
      Order_Date: new Date(),
      Lab_ID: "LAB_RISK_002",
      Product_ID: "PROD_002",
      Quantity: 50,
...
function pcaiSimulateStockOutStress_() {
  const inventory = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_INVENTORY);
  const orders = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_ORDERS);
  const ar = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_AR);
  ...
  [
    {
      ...
      Invoice_ID: "SIM_INV_2001",
      Invoice_Status: "Draft",
      Payment_Status: "Pending"
    },
    {
      Order_ID: "SIM_OD_2002",
      Order_Date: new Date(),
      Lab_ID: "LAB_2002",
      Product_ID: "PROD_002",
      Quantity: 20,
  ...
  [
    {
      ...
      Order_Total: 12500,
      Order_Status: "Delivered",
      Invoice_ID: "SIM_CS_INV_0001",
      Invoice_Status: "Sent",
      Payment_Status: "Pending"
    },
    {
      Order_ID: "SIM_CS_0002",
      Order_Date: new Date(),
      Lab_ID: "LAB_BIG_001",
      Product_ID: "PROD_002",
      Quantity: 40,
      ...
      Order_Total: 1600,
      Order_Status: "Delivered",
      Invoice_ID: "SIM_CS_INV_0002",
      Invoice_Status: "Sent",
      Payment_Status: "Pending"
    },
    {
      Order_ID: "SIM_CS_0003",
      Order_Date: new Date(),
      Lab_ID: "LAB_BIG_002",
      Product_ID: "PROD_001",
      Quantity: 18,
      ...
      Order_Total: 9000,
      Order_Status: "Delivered",
      Invoice_ID: "SIM_CS_INV_0003",
      Invoice_Status: "Sent",
      Payment_Status: "Pending"
    },
    {
      Order_ID: "SIM_CS_0004",
      Order_Date: new Date(),
      Lab_ID: "LAB_BIG_003",
      Product_ID: "PROD_003",
      Quantity: 22,
      ...
      Order_Total: 2640,
      Order_Status: "Delivered",
      Invoice_ID: "SIM_CS_INV_0004",
      Invoice_Status: "Draft",
      Payment_Status: "Pending"
    },
    {
      Order_ID: "SIM_CS_0005",
      Order_Date: new Date(),
      Lab_ID: "LAB_BIG_002",
      Product_ID: "PROD_002",
      Quantity: 30,
...
function pcaiSetupScenarioBenchmarks() {
  const sh = pcaiResetSheetContents_(pcaiGetOrCreateSheetSafe_(PCAI_SHEETS.SCENARIO_BENCHMARKS));
...
function pcaiGetBenchmarkRow_(scenarioName) {
  const sh = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.SCENARIO_BENCHMARKS);
  ...
  const match = rows.find(r =>
    String(r[map.Scenario_Name] || "").trim().toLowerCase() === String(scenarioName || "").trim().toLowerCase()
  );

  if (!match) {
    throw new Error("Benchmark not found for scenario: " + scenarioName);
  }
...
function pcaiVerifySandboxDashboard_(scenarioName) {
  const sh = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_DASHBOARD);
  sh.getRange(4, 1, Math.max(sh.getMaxRows() - 3, 1), 5).clearContent();

  const benchmark = pcaiGetBenchmarkRow_(scenarioName);
  const orders = pcaiGetRowsAsObjects_(pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_ORDERS), "Order_ID");
  const ar = pcaiGetRowsAsObjects_(pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_AR), "Lab_ID");
  const inventory = pcaiGetRowsAsObjects_(pcaiGetSheetRequiredSafe_(PCAI_SHEETS.TEST_INVENTORY), "Product_ID");

  const totalRevenue = orders.reduce((sum, r) => sum + pcaiNum_(r.Order_Total || r.Total_Amount || 0), 0);
  const unpaidCount = orders.filter(r => String(r.Payment_Status || "").trim() !== "Received").length;
  const reorderCount = inventory.filter(r => String(r.Reorder_Status || "").trim().toUpperCase() === "REORDER").length;

  const revenueByLab = {};
  orders.forEach(r => {
    const lab = String(r.Lab_ID || "").trim();
    if (!lab) return;
    revenueByLab[lab] = (revenueByLab[lab] || 0) + pcaiNum_(r.Order_Total || r.Total_Amount || 0);
  });

  const sortedLabs = Object.keys(revenueByLab)
    .map(k => ({ lab: k, revenue: revenueByLab[k] }))
    .sort((a, b) => b.revenue - a.revenue);

  const topLab = sortedLabs.length ? sortedLabs[0].lab : "";
  const topLabRevenue = sortedLabs.length ? sortedLabs[0].revenue : 0;
  const creditHoldCount = ar.filter(r => String(r.Credit_Hold || "").trim().toUpperCase() === "HOLD").length;
  ...
  const checks = [
    [
      ...
      scenarioName,
      pcaiCompareValues_(scenarioName, benchmark.Scenario_Name) ? "PASS" : "FAIL",
      "Scenario name recorded"
    ],
    [
      "Total Revenue",
      benchmark.Expected_Revenue,
      totalRevenue,
      pcaiCompareValues_(totalRevenue, benchmark.Expected_Revenue) ? "PASS" : "FAIL",
      "Sum of TEST_Orders revenue"
    ],
    [
      "Unpaid Count",
      benchmark.Expected_Unpaid_Count,
      unpaidCount,
      pcaiCompareValues_(unpaidCount, benchmark.Expected_Unpaid_Count) ? "PASS" : "FAIL",
      "Count of Payment_Status != Received"
    ],
    [
      "Reorder Count",
      benchmark.Expected_Reorder_Count,
      reorderCount,
      pcaiCompareValues_(reorderCount, benchmark.Expected_Reorder_Count) ? "PASS" : "FAIL",
      "Count of TEST_Inventory.Reorder_Status = REORDER"
    ],
    [
      "Top Lab",
      benchmark.Expected_Top_Lab,
      topLab,
      pcaiCompareValues_(topLab, benchmark.Expected_Top_Lab) ? "PASS" : "FAIL",
      "Highest revenue lab"
    ],
    [
      "Top Lab Revenue",
      benchmark.Expected_Top_Lab_Revenue,
      topLabRevenue,
      pcaiCompareValues_(topLabRevenue, benchmark.Expected_Top_Lab_Revenue) ? "PASS" : "FAIL",
      "Revenue of highest revenue lab"
    ],
    [
      "Credit Holds Present",
      benchmark.Expected_Credit_Holds,
      creditHoldCount,
      pcaiCompareValues_(creditHoldCount, benchmark.Expected_Credit_Holds) ? "PASS" : "FAIL",
      "Count HOLD in TEST_AR_Credit_Control"
    ]
  ];
...
function pcaiCreateRegressionSummarySheet_() {
  const sh = pcaiResetSheetContents_(pcaiGetOrCreateSheetSafe_(PCAI_SHEETS.REGRESSION_SUMMARY));

  sh.getRange("A1")
    .setValue("PrimeCare Regression Summary")
    .setFontWeight("bold")
    .setFontSize(16);
...
function pcaiRunAllSandboxScenarios() {
  ...
  if (out.length) {
    sh.getRange(4, 1, out.length, 8).setValues(out);
  }

  sh.autoResizeColumns(1, 8);
  pcaiLogActionSafe_("RUN_ALL_SANDBOX_SCENARIOS", "Completed full regression suite");
  return "All sandbox scenarios executed.";
}

function pcaiGetScenarioTestStatus_() {
  if (!pcaiSheetExistsSafe_(PCAI_SHEETS.REGRESSION_SUMMARY)) return "NOT RUN";

  const sh = pcaiGetSheetRequiredSafe_(PCAI_SHEETS.REGRESSION_SUMMARY);
  const values = sh.getDataRange().getValues();
