...
/** Module IDs that may surface scoped blockers (dashboard shows all). */
export const PEOPLE_OPS_BLOCKER_MODULES = Object.freeze([
  "employees",
  "compensation",
  "payroll",
  "ownership",
...
function scopedWarning(moduleIds, payload) {
  return { ...payload, moduleIds: [...moduleIds] };
}
...
export function buildPeopleOpsDataQualityWarnings({
  ...
  if (withoutPlan.length) {
    warnings.push(
      scopedWarning(["compensation"], {
        id: "missing-assignments",
        severity: "warning",
        blockerLabel: "Assignment Blocker",
  ...
  const incompleteProfiles = employees.filter(
    (row) => !str(row.profileUserId) || !str(row.employeeName) || !str(row.role)
  );
  if (incompleteProfiles.length) {
    warnings.push(
      scopedWarning(["employees"], {
        id: "missing-profiles",
        severity: "attention",
        blockerLabel: "Directory Attention",
        title: `${incompleteProfiles.length} employee profile${incompleteProfiles.length === 1 ? "" : "s"} need more information.`,
        detail: "Missing name, role, or profile link can block compensation assignment and payroll.",
        why: "Complete employee profiles keep directory, compensation, and payroll aligned.",
...
/** Dashboard shows all blockers; module pages show only module-scoped items. */
export function filterPeopleOpsDataQualityWarningsForModule(warnings = [], moduleId = "") {
  const scope = str(moduleId);
  if (!scope || scope === "dashboard") return warnings;
  return (warnings || []).filter((row) => (row.moduleIds || []).includes(scope));
}

/**
 * Compact cross-module dependency cards — one-line context + CTA, not full enterprise banners.
 */
export function buildPeopleOpsModuleDependencyNotices({ moduleId = "", employeeList = [] } = {}) {
  const scope = str(moduleId);
  const notices = [];
  ...
  if (scope === "payroll") {
    ...
    if (withoutPlan.length) {
      notices.push({
        id: "payroll-needs-compensation",
        title: `Payroll cannot continue because ${withoutPlan.length} employee${withoutPlan.length === 1 ? "" : "s"} have no compensation plan.`,
        actionLabel: "Go to Compensation",
        actionRoute: { moduleId: "compensation", screenId: "assignments" },
      });
    }
  }
  ...
  if (scope === "budgeting") {
    ...
    if (withoutPlan.length) {
      notices.push({
        id: "budget-needs-compensation",
        title: `${withoutPlan.length} employee${withoutPlan.length === 1 ? "" : "s"} still need compensation plans before payroll can anchor the budget.`,
        actionLabel: "Go to Compensation",
        actionRoute: { moduleId: "compensation", screenId: "assignments" },
      });
    }
