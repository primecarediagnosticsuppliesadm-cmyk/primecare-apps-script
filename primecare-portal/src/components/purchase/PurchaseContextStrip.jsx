...
/**
 * Compact Purchase orientation strip — view, PO, supplier, search, filters, sort, freeze.
 */
export default function PurchaseContextStrip({ parts = [], warning = "", className }) {
  const visible = (parts || []).filter(Boolean);
  if (!visible.length && !warning) return null;

  return (
    <div className={cn("space-y-1.5", className)} data-purchase-context-strip="true">
      {visible.length ? (
        <p
