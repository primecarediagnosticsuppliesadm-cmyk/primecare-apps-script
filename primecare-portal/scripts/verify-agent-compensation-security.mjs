...
assert(agent.agentOwnProfileOnly && !agent.canChangePlan, "role.agent_future", "Agent own-profile contract declared");
...
assertThrows(
  () => assertAgentCompensation360Access("lab", { targetAgentId: "A1" }),
  "guard.lab",
  "Lab access blocked"
);
assertThrows(
  () => assertAgentCompensation360Access("agent", { actorAgentId: "A2", targetAgentId: "A1" }),
  "guard.agent_other",
  "Agent cannot view other profiles"
);
assert(
  assertAgentCompensation360Access("agent", { actorAgentId: "A1", targetAgentId: "A1" }),
  "guard.agent_own",
...
assert(/assertAgentCompensation360Access/.test(apiSrc), "api.guard", "360 API enforces access guard");
