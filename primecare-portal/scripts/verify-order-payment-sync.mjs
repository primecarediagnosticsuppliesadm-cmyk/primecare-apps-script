...
assert(/isHqOrderStatusWriteBlocked/.test(orders), "Orders uses HQ freeze policy");
...
assert(/Mark Processing/.test(ordersStatusActions) && /hqStatusWriteBlocked/.test(ordersStatusActions), "Status writes respect HQ freeze");
assert(!/disabled=\{hqFrozen\}[\s\S]{0,240}handleRecordOrderPayment/.test(orders), "Record Payment not disabled by HQ freeze");
