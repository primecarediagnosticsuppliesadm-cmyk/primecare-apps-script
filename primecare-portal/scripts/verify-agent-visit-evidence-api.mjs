...
const filterLabsFn = access.split("export function filterLabsForUser")[1]?.slice(0, 1800) || "";
assert(
  /status === "PROSPECT"/.test(filterLabsFn) && /return false/.test(filterLabsFn),
  "compat.filter_no_prospect",
  "filterLabsForUser still excludes PROSPECT; VE-2 does not broaden it into an include"
);
assert(
