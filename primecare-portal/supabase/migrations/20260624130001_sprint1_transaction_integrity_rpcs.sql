...
BEGIN
  ...
  IF NOT (
    public.can_write_ops_for_tenant(v_tid::uuid)
    OR (
      public.current_user_role() = 'agent'
      AND public.lab_record_is_visible_to_current_user(v_tid::uuid, v_lab)
    )
  ) THEN
...
BEGIN
  ...
  IF NOT public.can_write_ops_for_tenant(v_tid::uuid) THEN
    RAISE EXCEPTION 'forbidden';
  END IF;
