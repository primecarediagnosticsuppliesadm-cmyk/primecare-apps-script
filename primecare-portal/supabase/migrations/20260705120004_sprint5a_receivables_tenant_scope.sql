-- Sprint 5A — tenant-scope receivables projection reads (avoid cross-tenant seq scans).
...
BEGIN
  ...
  WHERE public.tenant_id_matches(p.tenant_id);
...
COMMENT ON FUNCTION public.read_lab_receivables_list_v1 IS
  'Lab receivables list from proj_lab_receivable_v1, tenant-scoped via tenant_id_matches (Sprint 5A).';
