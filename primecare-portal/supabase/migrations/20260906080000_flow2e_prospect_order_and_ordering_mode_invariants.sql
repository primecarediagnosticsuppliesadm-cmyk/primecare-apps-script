...
--
-- Smallest invariant hardening after Flow 2D:
--   * create_lab_order requires labs.status = ACTIVE for ALL callers (Lab, Admin, Executive).
--     Existing error: lab_inactive. Lab ordering_mode self-initiate rules unchanged.
--   * orders_insert_by_role HQ path also requires lab_row_is_active.
--   * PROSPECT rows cannot leave ordering_mode = hq_managed while remaining PROSPECT.
--     activate_prospect_lab (PROSPECT -> ACTIVE + hq_managed) is not blocked.
--
-- No pricing, inventory deduction, AR, fulfillment, or payment changes.
...
COMMENT ON FUNCTION public.create_lab_order(text, text, text, jsonb, text, date, text, text, text) IS
  'Creates a lab order. Lab identity comes from the authenticated profile. Line unit_price is always products.selling_price; client unit_price is ignored. Place validates stock; it does not deduct inventory. labs.status must be ACTIVE for every caller.';
...
CREATE OR REPLACE FUNCTION public.labs_prospect_ordering_hq_managed()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF upper(btrim(COALESCE(NEW.status, ''))) = 'PROSPECT'
       AND lower(btrim(COALESCE(NEW.ordering_mode, ''))) IS DISTINCT FROM 'hq_managed'
    THEN
      RAISE EXCEPTION 'prospect_ordering_hq_managed';
    END IF;
    RETURN NEW;
  END IF;
  ...
  IF TG_OP = 'UPDATE'
     ...
     AND upper(btrim(COALESCE(NEW.status, ''))) = 'PROSPECT'
     AND lower(btrim(COALESCE(NEW.ordering_mode, ''))) IS DISTINCT FROM 'hq_managed'
  THEN
    RAISE EXCEPTION 'prospect_ordering_hq_managed';
  END IF;
...
ALTER FUNCTION public.labs_prospect_ordering_hq_managed() OWNER TO postgres;
REVOKE ALL ON FUNCTION public.labs_prospect_ordering_hq_managed() FROM PUBLIC;
REVOKE ALL ON FUNCTION public.labs_prospect_ordering_hq_managed() FROM anon;
REVOKE ALL ON FUNCTION public.labs_prospect_ordering_hq_managed() FROM authenticated;

DROP TRIGGER IF EXISTS labs_prospect_ordering_hq_managed_trg ON public.labs;
CREATE TRIGGER labs_prospect_ordering_hq_managed_trg
  BEFORE INSERT OR UPDATE ON public.labs
  FOR EACH ROW
  EXECUTE FUNCTION public.labs_prospect_ordering_hq_managed();

COMMENT ON FUNCTION public.labs_prospect_ordering_hq_managed() IS
  'PROSPECT labs must keep ordering_mode = hq_managed until activate_prospect_lab moves status to ACTIVE.';
