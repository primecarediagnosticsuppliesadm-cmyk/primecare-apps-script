...
-- Idempotent. Does not drop legacy columns (id, title, message, payload, updated_at).
-- Does not grant writes to anon. Does not weaken RLS.
...
-- Backfill event_id from legacy id when present.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
EXCEPTION
  WHEN others THEN
    -- id may not be uuid-castable; fall through to random fill
    NULL;
END
$$;
...
-- Backfill payload_json from legacy payload when present.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
-- Ensure authenticated can use foundation + legacy columns under existing RLS.
