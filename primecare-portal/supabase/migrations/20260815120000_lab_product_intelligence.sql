...
-- Idempotent. Does not alter lab_qualifications 1:1 profile.

ALTER TABLE public.agent_visits
  ADD COLUMN IF NOT EXISTS next_follow_up_type text,
  ADD COLUMN IF NOT EXISTS next_action text;

COMMENT ON COLUMN public.agent_visits.next_follow_up_type IS
  'Follow-up channel: Call, Visit, Send Pricing, Demo, Collection.';
COMMENT ON COLUMN public.agent_visits.next_action IS
  'Next operational action captured on the visit follow-up step.';
...
COMMENT ON TABLE public.lab_product_intelligence IS
  'Year-1 incumbent product mix per lab. Market discovery — not PrimeCare warehouse stock.';
