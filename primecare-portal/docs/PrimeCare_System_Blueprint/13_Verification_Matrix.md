...
| verify-agent-visit-evidence-api.mjs | **VE-2:** canonical `createAgentVisitWrite` reuse; legacy payload; header+lines; bounded reads; spoof/cross-tenant; snapshot/finance firewall. `--remote` is QA URL only | Visit Evidence |
| verify-agent-visit-evidence-ux.mjs | **VE-3 entry UAT:** Labs Start Visit / Prospect Log Visit must land on fast form; wizard draft skipped on new intent; Add ANALYZER/REAGENT/CONSUMABLE stay fast; Qualify still opens wizard. `--remote` QA only; cleanup `[VE-3-CERT]` | Visit Evidence |
