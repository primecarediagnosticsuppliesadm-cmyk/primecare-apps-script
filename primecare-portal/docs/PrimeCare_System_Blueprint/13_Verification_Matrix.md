...
| verify-prospect-email-1b2.mjs | PN-1B2 dispatcher: cron-secret auth, no open relay, QA suppression, claim SKIP LOCKED, Resend idempotency key, retry policy, EMAIL_ENABLED no-claim. Default static; `--apply` QA only | Notifications / Prospect email |
