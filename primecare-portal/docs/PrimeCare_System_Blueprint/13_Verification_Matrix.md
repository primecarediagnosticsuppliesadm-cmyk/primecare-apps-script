...
| verify-lab-ordering-1h-ar-and-projection.mjs | `authenticated` AR UPDATE grant; anon has no AR UPDATE; RLS tenant-scopes AR writes; Lab cannot HQ AR update; exactly one projection signature per worker; RPC with `p_cascade_metrics`; `updateOrderStatusWrite` Placed→Fulfilled posts merchandise AR | Lab Ordering 1H AR grant + projection overload drop (QA) |
...
| verify-agent-prospect-2a.mjs | Agent `create_prospect_lab` PROSPECT create; identity/tenant/sourced_by derivation; no AR/ownership/lab user; spoof + role rejects; sourced_by immutability; same-tenant duplicates; visibility; HQ `create_lab_with_ar_credit` regression; PN-1A `prospect_created` (exactly one event, duplicate create does not emit, client forge denied, HQ visible / Lab+HR hidden). Default static; `--apply` QA only | Agent Prospect 2A |
...
| verify-agent-prospect-2c.mjs | HQ `activate_prospect_lab`; AR once; optional ownership; sourced_by immutable; repeat activation reject; role/tenant denies; generic PATCH cannot PROSPECT→ACTIVE; HQ UI Activate Lab + Prospects filter; PN-1A `prospect_activated` (sourcing Agent only, other Agent/Lab hidden, retry no second event, client forge denied). Default static; `--apply` QA only | Agent Prospect 2C |
| verify-agent-prospect-2e.mjs | Flow 2E: `create_lab_order` ACTIVE for all callers; PROSPECT cannot receive orders; PROSPECT `ordering_mode` stays `hq_managed`; activation + ACTIVE HQ ordering regression. Default static; `--apply` QA only | Agent Prospect 2E |
...
| verify-agent-visit-evidence-rls.mjs | **VE-1:** identity + lab visibility; sourced PROSPECT visit; spoof denied; Lab/HR denied. `--remote` is QA URL only | Visit Evidence |
| verify-agent-visit-evidence-api.mjs | **VE-2:** canonical `createAgentVisitWrite` reuse; legacy payload; header+lines; **idempotent same-visit line UUID retry**; bounded reads; spoof/cross-tenant; snapshot/finance firewall. `--remote` is QA URL only | Visit Evidence |
| verify-agent-visit-evidence-ux.mjs | **VE-3 entry UAT + P1:** follow-up type omitted without a date; compact discovery lines drop whitespace/invalid-only rows and keep explicit `0`; Labs Start Visit / Prospect Log Visit land on fast form; wizard draft skipped on new intent; Add ANALYZER/REAGENT/CONSUMABLE stay fast; **Deep qualification** opens wizard; `--remote` QA only; cleanup `[VE-3-CERT]` / `[VE-3-P1-CERT]` | Visit Evidence |
| verify-notification-contract.mjs | Notification payload allowlist, no legacy delivery columns, canonical client, no RETURNING SELECT RLS trap, fire-and-forget; PN-1A `prospect_created`/`prospect_activated` templates, server-only insert, unique index, Flow 2 emit hooks, bounded reads; PN-1B1 queue twin + no-send | Notifications |
| verify-prospect-email-1b1.mjs | PN-1B1 email queue foundation: schema/CHECKs, enqueue trigger, HQ/Agent recipient rules, unique delivery, client INSERT/UPDATE denied, no provider/send. Default static; `--apply` QA only | Notifications / Prospect email |
| verify-prospect-email-1b2.mjs | PN-1B2 dispatcher + PN-1B3A QA rewrite: cron-secret auth, no open relay, `@primecare.test` rewrites to `EMAIL_TEST_RECIPIENT`, domain allowlist is not provider-deliverable, claim SKIP LOCKED, Resend idempotency, EMAIL_ENABLED no-claim. Default static; `--apply` QA only | Notifications / Prospect email |
...
| verify-deploy-commit.mjs | Local HEAD vs origin/qa\|main; build stamp wiring; browser SHA vs canonical Production URL | Deploy identity |
...
| Agent prospect 2B | Agent Add Prospect; success copy is HQ review not ACTIVE; prospect card has no credit/visit/payment; assigned ACTIVE cards unchanged; Admin still uses HQ Add Lab |
