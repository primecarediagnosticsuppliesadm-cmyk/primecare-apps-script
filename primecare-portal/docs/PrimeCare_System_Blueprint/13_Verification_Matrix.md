...
| verify-notification-contract.mjs | Notification payload allowlist, no legacy delivery columns, canonical client, no RETURNING SELECT RLS trap, fire-and-forget; PN-1A `prospect_created`/`prospect_activated` templates, server-only insert, unique index, Flow 2 emit hooks, bounded reads | Notifications |
